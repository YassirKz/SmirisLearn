...
export default function GroupForm({ isOpen, onClose, onSuccess, group }) {
  ...
  const validate = () => {
    ...
    else if (formData.name.trim().length < 3) errs.name = 'Minimum 3 caractères';
    else if (formData.name.trim().length > 100) errs.name = 'Maximum 100 caractères';
    return errs;
  };
              ...
              <form onSubmit={handleSubmit} className="space-y-5">
                ...
                <div>
                  ...
                  <textarea
                    ...
                    rows={3}
                    maxLength={500}
                    className="w-full px-4 py-2 border border-gray-200 rounded-xl focus:border-indigo-400 focus:ring-4 focus:ring-indigo-100 outline-none transition-all resize-none"
                  />
                </div>
                  ...
                  >
                    {loading ? (
                      ...
                      <>
                        ...
                        <span>{group ? 'Modifier' : 'Créer'}</span>
                      </>
                    )}
                  </button>
                </div>
